...
void detectAndDisplay(Mat frame)
{
	...
	if (faces.size() > 0)
	{
		...
		ratio = (faces[0].width*faces[0].height / 1000)-20;
		//printf("%f \n", ratio);
	...
	for (size_t j = 0; j < eyes.size(); j++)
	{
		...
		Point swp(eyes[j].x, eyes[j].y);
		Point nep(eyes[j].x + eyes[j].width*0.5, eyes[j].y+eyes[j].height*0.5);
		...
		rectangle(frame, swp, nep, Scalar(255, 0, 0), 4, 8, 0);
